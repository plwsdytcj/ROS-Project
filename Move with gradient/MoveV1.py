...
class myMoveG:
	def __init__(self,goalx1,goaly1):
		#set the stepsize to be 0.5,which means the size of the grid is 0.5*0.5
		self.stepsize=0.5
	...
	def start(self):
		...
		while not rospy.is_shutdown():
			...
			self.reachTarget()	
			# if it reaches the goal cell, stop
	...
	def updateTwist(self):
		...
		if(self.nextLocalGoalIndex==0):
			...
			# if the index is 1, that means the local goal cell is the upper right
			if(currentAngle>math.radians(-135)):
				# if the current orientation is greater than -135, we set the angular.z to be positive, so it's more time efficient
				self.vx=0.2
				a=self.currentPose.pose.pose.position.x-((self.currentCellx+1)*self.stepsize-self.stepsize/2)
				...
				#if the current orientation is less than -135, we set the angular.z to be negative, so it's more time efficient
				self.vx=0.2
				a=self.currentPose.pose.pose.position.x-((self.currentCellx+1)*self.stepsize-self.stepsize/2)
				b=self.currentPose.pose.pose.position.y-((self.currentCelly+1)*self.stepsize-self.stepsize/2)
				distance=math.sqrt(a*a+b*b)
			...
			# if the index is 2, that means the local goal cell is the up one
			...
			#if the index is 3, that means the local goal cell is the upper left
			if(currentAngle>math.radians(-45)):
				# if the current orientation is greater than -45, we set the angular.z to be positive, so it's more time efficient
				self.vx=0.2
				a=self.currentPose.pose.pose.position.x-((self.currentCellx-1)*self.stepsize-self.stepsize/2)
				b=self.currentPose.pose.pose.position.y-((self.currentCelly+1)*self.stepsize-self.stepsize/2)
				distance=math.sqrt(a*a+b*b)
				...
				# if the current orientation is less than -45, we set the angular.z to be positive, so it's more time efficient
				self.vx=0.2
				a=self.currentPose.pose.pose.position.x-((self.currentCellx-1)*self.stepsize-self.stepsize/2)
				b=self.currentPose.pose.pose.position.y-((self.currentCelly+1)*self.stepsize-self.stepsize/2)
				distance=math.sqrt(a*a+b*b)
			...
			# if the current orientation is less than 135, we set the angular.z to be positive, so it's more time efficient
				self.vx=0.2
				a=self.currentPose.pose.pose.position.x-((self.currentCellx+1)*self.stepsize-self.stepsize/2)
				b=self.currentPose.pose.pose.position.y-((self.currentCelly-1)*self.stepsize-self.stepsize/2)
				distance=math.sqrt(a*a+b*b)
	...
	def normalize_angle(angle):
			#normalize the angle since the radians are between pi to -pi
    		res = angle
    		while res > pi:
        		res -= 2.0 * pi
    		while res < -pi:
